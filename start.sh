...
(sudo tail -f -n0 $LOG_FILE | stdbuf -o0 bgrep -b \"\<KP4\>\" | while read -r; do echo ===== Playing Mister Rogers =====; ps ax | grep castnow | grep node | awk '{print $1}' | xargs kill; youtube-dl --playlist-random -o - https://www.youtube.com/playlist?list=PLf22vOV7unX-ly5LC98UyZTOSG8ckDqmt | castnow --device "$CHROMECAST_DEV" --quiet -; done) &
(sudo tail -f -n0 $LOG_FILE | stdbuf -o0 bgrep -b \"\<KP5\>\" | while read -r; do echo ===== Playing Daniel Tiger =====; ps ax | grep castnow | grep node | awk '{print $1}' | xargs kill; youtube-dl --playlist-random -o - https://www.youtube.com/playlist?list=PLKzJHcOwWn3KWJco0w6gVvhcrhXDb4TFi | castnow --device "$CHROMECAST_DEV" --quiet -; done) &
(sudo tail -f -n0 $LOG_FILE | stdbuf -o0 bgrep -b \"\<KP6\>\" | while read -r; do echo ===== Playing Thomas and Friends =====; ps ax | grep castnow | grep node | awk '{print $1}' | xargs kill; youtube-dl --playlist-random -o - https://www.youtube.com/playlist?list=PLI2i4PrLia3gAAYjMaVbAwdt1xrOsUaXS | castnow --device "$CHROMECAST_DEV" --quiet -; done) &
...
(sudo tail -f -n0 $LOG_FILE | stdbuf -o0 bgrep -b \"\<KP/\>\" | while read -r; do echo ===== Playing OSU Marching Band =====; ps ax | grep castnow | grep node | awk '{print $1}' | xargs kill; youtube-dl --playlist-random -o - https://www.youtube.com/playlist?list=PLyXb8EZU88Q-qSxRiLOzTcn1Rer7VMgJa | castnow --device "$CHROMECAST_DEV" --quiet -; done) &
